...
fn prototye_v1(message: u32, hwnd: HWND) {
    ...
    unsafe {
        let postmessage_result =
            PostMessageW(Some(hwnd), message, Default::default(), Default::default());
...
fn get_client_data() -> ClientData {
    ...
    let hwnd = usize::from_str_radix(args.get(1).expect("No HWND GIVEN!"), 10).unwrap(); // trust me
